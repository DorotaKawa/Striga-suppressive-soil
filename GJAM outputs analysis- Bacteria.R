...
BacCor2 <- list_all[grep("Bact.*2week", names(list_all))]
lapply(BacCor2, head, n=2)

BacCor3 <- list_all[grep("Bact.*3week", names(list_all))]
lapply(BacCor3, head, n=2)

#### Bacteria 2week - filter positive/negative correlations per trait
selcor2 <- function(x) {
  x %>% 
...
Bac2_list <- BacCor2 %>% lapply(selcor2)

#### Bacteria 3week - filter positive/negative correlations per trait
selcor3 <- function(x) {
  x %>% 
...
Bac3_list <- BacCor3 %>% lapply(selcor3)
Bac_list <- append(Bac2_list, Bac3_list)
...
names(tofL2) <- tof2
for(i in tof2){
  Bac <- Bac_list[sapply(Bac_list, contains_tof, nam = i)]
  t.sel_names <- c(sel_names, i)
  trait <- lapply(Bac, "[", t.sel_names)
...
# write.xlsx(tofL2, "Correlations 2 weeks per trait.xlsx")
...
# write.xlsx(tofL3, "Correlations 3 weeks per trait.xlsx")
...
### Select top 100 taxa to plot
top_2w <- Rank_allcomp2 %>%
  replace_with_na_at(vars(DMBQ_rank:Vanillic_rank), condition = ~.x > 100) 

top_3w <- Rank_allcomp3 %>%
  replace_with_na_at(vars(Sub_rank), condition = ~.x > 100)  

top_23w <- Rank_allcomp23 %>%
  replace_with_na_at(vars(Aer_rank:Att_rank), condition = ~.x > 100) 
...
for (i in tof2) {
  hmsel2List[[i]] <- top_2w %>%
    ...
    arrange(Phylum) %>%
    mutate(sum = rowSums(dplyr::select(., BactBK2week, BactRh2week, BactEndo2week), na.rm = TRUE)) %>%
    filter(sum != 0) %>%
    select(-sum) %>%
    select(term:Species, BactBK2week, BactRh2week, BactEndo2week)
  
}

pdf("Bact top 100 heatmap 2wpi.pdf")
for (i in tof2){
  hp <- hmsel2List[[i]]
...
for (i in tof3) {
  hmsel3List[[i]]  <- top_3w %>%
    ...
    arrange(Phylum) %>%
    mutate(sum = rowSums(dplyr::select(., BactBK3week, BactRh3week, BactEndo3week, BactEndoSand3week), na.rm = TRUE)) %>%
    filter(sum != 0) %>%
    select(-sum) %>%
    select(term:Species, BactBK3week, BactRh3week, BactEndo3week, BactEndoSand3week)
}

pdf("Bact top 100 heatmap 3wpi.pdf")
for (i in tof3){
  hp <- hmsel3List[[i]]
...
for (i in tof23) {
  hmsel23List[[i]] <- top_23w %>%
    ...
    arrange(Phylum) %>%
    mutate(sum = rowSums(dplyr::select(.,BactBK2week, BactRh2week, BactEndo2week, BactBK3week, BactRh3week, BactEndo3week, BactEndoSand3week), na.rm = TRUE)) %>%
    filter(sum != 0) %>%
    select(-sum) %>%
    select(term:Species,BactBK2week, BactRh2week, BactEndo2week,BactBK3week, BactRh3week, BactEndo3week, BactEndoSand3week)
}


pdf("Bact top 100 heatmap 2 and 3wpi.pdf")
for (i in tof23){
  hp <- hmsel23List[[i]]
...
Bac2_ranks <- Bac2_list %>% lapply (ranking2)
Bac3_ranks <- Bac3_list %>% lapply (ranking3)

Bac_ranks <- append(Bac2_ranks, Bac3_ranks)
# write.xlsx(Bac_ranks, "Ranks per trait*comp Bac.xlsx")
...
combo2 <- map2(BacCor2, Bac2_ranks, full_join)
lapply(combo2, head, n=2)

combo3 <- map2(BacCor3, Bac3_ranks, full_join)
...
pdf("3 Suberin combirank - Bacteria.pdf")
...
pdf("3 Aerenchyma combirank - Bacteria.pdf")
for(i in names(combo3)){
  print(
...
pdf("2 Aerenchyma combirank - Bacteria.pdf")
...
pdf("2 DMBQ combirank - Bacteria.pdf")
for(i in names(combo2)){
  print(
...
pdf("2 VA combirank - Bacteria.pdf")
for(i in names(combo2)){
  print(
...
pdf("2 SA combirank - Bacteria.pdf")
for(i in names(combo2)){
  print(
...
Bac2_list02 <- BacCor2 %>% lapply(selcor2_02)
...
Bac3_list02 <- BacCor3 %>% lapply(selcor3_02)

# Combiranks for taxa with correlations filtered at 0.2 or -0.2 and counts at the phylum level
Bac2_ranks02 <- Bac2_list02 %>% lapply (ranking2)
Bac3_ranks02 <- Bac3_list02 %>% lapply (ranking3)

combo2_02 <- map2(BacCor2, Bac2_ranks02, full_join)
lapply(combo2, head, n=2)

combo3_02 <- map2(BacCor3, Bac3_ranks02, full_join)
lapply(combo3, head, n=2)
...
pdf("Bar plots -combined rank2 - Bacteria.pdf")
...
pdf("Bar plots -combined rank3 - Bacteria.pdf")
...
pdf("Bar plots -combined rank3 - Bacteria Class .pdf")
...
pdf("Bar plots -combined rank2 - Bacteria Class .pdf")
...
pdf("Overlaps-0.2cutoff - Bacteria.pdf")
...
pdf("Bar plots -Intersects -Bacteria.pdf")
...
pdf("Bar plots -Intersects class - Bacteria.pdf")
...
pdf("Bar plots -Intersects wpi3 class Bacteria.pdf")
...
write.xlsx(tofL, "Bacteria Combined ranks trait*compartment.xlsx")

##############################################################Get rank values for the isolates collection###################################

#Load file with available isolates
iso <- read_csv("/Users/dorota/Dropbox/UC_DAVIS/PROMISE/POP#2/GJAM/Strain selection/NIOO Bacterial list from Clue field.csv")
iso$genus <- paste0("g",iso$genus)


In_collection <- Bac_ranks %>% lapply(function(x) {
  x %>% 
    filter(Genus %in% iso$genus)
}
)

write.xlsx(In_collection, "Isolates ranks.xlsx")

In_collection_corr <- Bac_list %>% lapply(function(x) {
  x %>% 
    filter(Genus %in% iso$genus)
}
)

Iso <- map2(In_colection, In_collection_corr, full_join)

write.xlsx(Iso, "Isolates  ranks and correlations.xlsx")
